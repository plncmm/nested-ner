...
def create_dataset_from_conll(train_path, test_path, dev_path, dataset):
    ...
    dtrain = {'data': x_train, 'labels': y_train, 'chars': x_chars_train, 'size': len(x_train)}
    dval = {'data': x_val, 'labels': y_val, 'chars': x_chars_val, 'size': len(x_val)}
    dtest = {'data': x_test, 'labels': y_test, 'chars': x_chars_test, 'size': len(x_test)}
    dtrain_full = {'data': x_train + x_val, 'labels': y_train + y_val, 'chars': x_chars_train + x_chars_val, 'size': len(x_train) + len(x_val)}
    return dtrain, dval, dtest, dtrain_full, vocab, char_vocab, vectorizer.tag_to_index, vectorizer.index_to_word, vectorizer